// components/ThreeDSurveyProductDetailPage.jsx
...
const ThreeDSurveyProductDetailPage = ({ product }) => {
  ...
  return (
    <div className="bg-black text-white py-20 flex flex-col items-center gap-24">
      ...
      {/* Satın Al Butonu title'ın hemen ardından */}
      {buyUrl && (
        <button
          onClick={() => window.open(buyUrl, "_blank")}
          className="px-8 py-3 text-lg md:text-xl font-semibold text-white bg-white/10 border border-white/20 rounded-xl shadow-md backdrop-blur-md hover:bg-blue-600 hover:border-blue-600 hover:scale-105 transition-all duration-300"
        >
          Satın Al
        </button>
      )}
