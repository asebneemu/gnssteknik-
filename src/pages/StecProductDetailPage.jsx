    ...
    <div className="bg-black text-white flex flex-col items-center gap-20 py-20">
      ...
      {paragraphSpecs && (
        ...
        >
          ...
          <div className="w-full md:w-[50%] bg-black bg-opacity-60 p-6 md:absolute md:right-0 md:top-12 md:bottom-8 h-auto flex flex-col justify-center items-end rounded-lg mt-6 md:mt-0">
            {renderMedia(images[3], "Sağdaki Görsel", "h-20 mb-4 object-contain")}
            <p className="text-left text-lg leading-relaxed">{paragraphSpecs}</p>
          </div>
        </motion.div>
      )}
      ...
      {(description || detailSpec) && (
        <motion.div
          className="w-full max-w-7xl flex flex-col lg:flex-row items-center justify-center py-20 px-4 relative gap-10"
          initial={{ opacity: 0, y: 50 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7 }}
          viewport={{ once: true }}
        >
          ...
          <div className="order-1 lg:order-2 w-full lg:w-[50%]">
            {renderMedia(images[4], "Ana Medya", "w-full h-auto object-cover rounded-xl")}
          </div>
        </motion.div>
      )}
        ...
        >
          <div className="flex flex-col gap-10 text-left">
            {metricSpecs.map((spec, i) => (
              <h2 key={i} className="text-3xl md:text-4xl font-extrabold leading-tight">{spec}</h2>
            ))}
          </div>
          <div className="relative w-full md:w-[600px] h-[300px] md:h-[380px]">
      ...
      {remainingSpecs.length > 0 && (
        ...
        >
          <div className="w-full lg:w-[50%]">
            {renderMedia(images[10], "Alt Medya", "w-full h-auto object-cover rounded-lg")}
          </div>
